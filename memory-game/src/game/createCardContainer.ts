...
export function createCardContainer(tex: Texture, w: number, h: number, radius = 8): Container {
  ...
  const border = new Graphics().roundRect(0, 0, w, h, radius).stroke({ width: 8, color: 0xffffff })
...
export function createCardContainerWithGradient(
  ...
  const border = new Graphics().roundRect(0, 0, w, h, radius).stroke({ width: 4, color: 0xffffff })
